...
> - **Slices compress.** `trace` is **10.3x** smaller than reading the files it draws from on Django, 4.2x here — measured over slices that found neighbours, against whole files, which is what an agent without an index actually reads. See [`docs/trace.md`](docs/trace.md).
>
...
3. **Raise call resolution.** 54.7% of Django's functions get at least one resolved neighbour, against 90.5% here; at the individual-edge level this repository resolves 32.1% of `CALLS`, up from 23.5% since the `src/` layout defect was fixed. Claims about a whole path being complete want the edge number. Dynamic dispatch, callables passed as arguments, registry lookups and metaclass machinery produce no edge, so a slice is a lower bound on the real path. Two options worth comparing rather than assuming: fix the resolver, or let an external Tree-sitter graph supply edges and keep Code Steward as the slicer and DRY pass. The `_module_key` src-layout defect is fixed (2026-08-23): it alone took `TESTED_BY` on this repo's own `src/` from 0 functions to 45.
