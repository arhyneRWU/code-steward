...
ALIAS_TABLES = (ABBREVIATIONS, PROGRAMMING_ALIASES)
STRATEGIES = (
    "baseline",
    "alias_expansion",
    "similarity_cap",
...
def search_with_aliases(
    ...
    """Fuse baseline search with conservative alias bonuses."""
...
def similarity_cap(results: list[SearchResult], limit: int) -> list[SearchResult]:
    """Compress top-N by removing metadata near-duplicates."""
...
def _retrieve(
    ...
    if strategy == "baseline":
        return search_units(units, query, limit, input_types, return_type)

    if strategy == "alias_expansion":
        return search_with_aliases(units, query, limit, input_types, return_type)

    if strategy == "similarity_cap":
        ranked = search_units(units, query, len(units), input_types, return_type)
        return similarity_cap(ranked, limit)

    if strategy == "alias_expansion_similarity_cap":
        ranked = search_with_aliases(
            units,
            query,
            len(units),
            input_types,
...
def evaluate_strategy(name: str) -> dict[str, Any]:
    """Evaluate one combined strategy on frozen Benchmark v1."""
    ...
    return {
        ...
        "summary": {
            ...
            "known_traps_returned": total_traps,
            "known_trap_rate": total_traps / total_candidates if total_candidates else 0.0,
            "known_redundant_candidates": total_redundant,
...
def run_comparison() -> dict[str, Any]:
    """Run four strategies without changing Benchmark v1."""
    return {
        "schema_version": 1,
        "experiment": "retrieval-combined-v1",
        "expansion_gain_weight": EXPANSION_GAIN_WEIGHT,
        "similarity_threshold": SIMILARITY_THRESHOLD,
        "alias_sources": ["abbreviations", "programming_aliases"],
